...
class IMUOdometry : public Odometry {
	public:
	/**
	 * IMUOdometry. Tracks the movement of the robot and estimates its position in
	 ...
	 * @param iimu The imu used for odometry
	 */
	IMUOdometry(
	    const TimeUtil& itimeUtil,
	    const std::shared_ptr<ReadOnlyChassisModel>& imodel, const IMU& iimu,
	    const ChassisScales& ichassisScales,
	    const std::shared_ptr<Logger>& ilogger = Logger::getDefaultLogger());

	virtual ~IMUOdometry() = default;
	 ...
	 * @param idTheta The angle difference from the previous step to this step.
	...
	virtual OdomState odomMathStep(const std::valarray<std::int32_t>& itickDiff,
	                               const QAngle& idTheta, const QTime& ideltaT);
};
